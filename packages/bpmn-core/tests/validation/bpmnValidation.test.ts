...
describe("BPMN validation", () => {
  ...
  it("flags a boundary-completion end distinctly from an ordinary implicit end", () => {
    const result = validateBpmnModel(
      model(
        [
          node("Start_1", "startEvent", "Start"),
          node("Task_1", "task", "Do work"),
          node("End_1", "endEvent", "End"),
          node("Boundary_Task_1_deadline", "boundaryEvent", ""),
          node("EndEvent_implicit_Boundary_Task_1_deadline", "endEvent", ""),
        ],
        [
          flow("Flow_1", "Start_1", "Task_1"),
          flow("Flow_2", "Task_1", "End_1"),
          flow("Flow_3", "Boundary_Task_1_deadline", "EndEvent_implicit_Boundary_Task_1_deadline"),
        ],
      ),
    );

    const found = codes(result);
    expect(found).toEqual(expect.arrayContaining(["IMPLICIT_BOUNDARY_END_EVENT"]));
    // The only implicit end here is boundary-derived, so the generic code must not fire,
    // and the dangling boundary must not be reported as a dead end / unreachable branch.
    expect(found).not.toContain("IMPLICIT_END_EVENT");
    expect(found).not.toContain("DEAD_END_NODE");
    expect(found).not.toContain("BRANCH_WITHOUT_END_REACHABILITY");
