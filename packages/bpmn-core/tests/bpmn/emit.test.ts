...
describe("emitBpmnXml — message end event", () => {
  it("emits a messageEventDefinition inside a terminal message end event", async () => {
    const src = "Customer: Place order\nShop: Confirm order\n(message Confirmation sent)\n";
    const result = parseDsl(src);
    const xml = emitBpmnXml(result.model);
    // The end event carries a message event definition (a BPMN message end event).
    expect(xml).toMatch(/<bpmn:endEvent[^>]*>[\s\S]*?<bpmn:messageEventDefinition[\s>/][\s\S]*?<\/bpmn:endEvent>/);
    await expect(importViaModdle(xml)).resolves.toBeUndefined();
  });
});
