...
export function parse(source: string): Program {
  ...
  for (const traceNode of traceNodes) {
    ...
    // Each CST `trace` is one blank-line-delimited paragraph. Per the DSL spec
    // (special-character cheat sheet: a blank line "breaks default pool scope"),
    // a stand-alone `Name:` default pool does NOT carry across a blank line into
    // the next paragraph — so every paragraph starts with no default. Without
    // this reset, an unprefixed opening line (e.g. a shared `(start …)` event)
    // inherits the previous paragraph's default lane, creating a spurious
    // cross-lane control-flow edge that fuses unrelated pools under union-find.
    const built = buildTraceFromTokens(tokens, "");
