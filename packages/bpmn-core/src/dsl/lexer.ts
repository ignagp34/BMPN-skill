...
import type { DeclaredPool, DslError, EventSubtype, TaskType } from "./ast.js";
...
export type LexResult = {
  tokens: LineToken[];
  errors: DslError[];
  /** Pool declarations from a trailing `== pools ==` block, if present (V5). */
  declaredPools?: DeclaredPool[];
};

// Pool/lane prefix `Name: rest`. The name may start with any letter (upper or
// lower case) and may contain dots, so dotted identifiers like `src.train:` or
// lowercase lane labels parse as a pool prefix rather than being swallowed into
// a task label.
const POOL_PREFIX_RE = /^(\p{L}[\p{L}\p{N}_ .\-/]*?):(?:\s+(.*))?$/u;
/**
 * Trailing pool-map block header (V5). Matches `== pools ==` (and `== pool ==`),
 * case-insensitive, with flexible spacing. Once seen, every subsequent line is
 * treated as a `PoolName -> lane; lane` declaration rather than a trace line.
 */
const POOL_HEADER_RE = /^\s*==\s*pools?\s*==\s*$/i;
...
function classifyLine(rawLine: string, lineNo: number, errors: DslError[]): LinePayload {
  ...
  const typedPoolMatch = /^([a-z]+)\s+(\p{L}[\p{L}\p{N}_ .\-/]*?):\s+(.+)$/u.exec(rawLine);
...
/**
 * Parse one line of a `== pools ==` block, of the form `PoolName -> a; b; c`.
 * Returns the declaration, or null (plus a LEX-1 warning) if malformed. Lanes
 * are split on `;`, trimmed, and empties dropped — the same exact-name
 * discipline the DSL uses elsewhere applies to matching these against inline
 * swimlane labels (validated downstream).
 */
function parsePoolEntry(raw: string, lineNo: number, errors: DslError[]): DeclaredPool | null {
  ...
  if (arrow === -1) {
    errors.push({
      ...
      code: "LEX-1",
      severity: "warning",
      message: `Pool declaration '${raw.trim()}' is missing '->'. Expected 'PoolName -> lane; lane; lane'.`,
    });
    return null;
  }
  ...
  if (name.length === 0) {
    errors.push({
      ...
      code: "LEX-1",
      severity: "warning",
      message: "Pool declaration has no pool name before '->'.",
    });
    return null;
  }
  if (lanes.length === 0) {
    errors.push({
      ...
      code: "LEX-1",
      severity: "warning",
      message: `Pool '${name}' declares no lanes after '->'.`,
    });
    return null;
  }
  return { name, lanes, line: lineNo };
}
...
export function tokenize(source: string): LexResult {
  ...
  const declaredPools: DeclaredPool[] = [];
  // Once the `== pools ==` header is seen, the remainder of the document is the
  // trailing pool-map block: its lines are parsed into declarations and hidden
  // from the parser (emitted as Blank tokens) so the trace grammar is untouched.
  let inPoolBlock = false;
  let offset = 0;
  for (let i = 0; i < lines.length; i++) {
    const raw = lines[i];
    const sourceLine = lineMap[i];
    let payload: LinePayload;
    if (POOL_HEADER_RE.test(raw)) {
      inPoolBlock = true;
      payload = { kind: "Blank" };
    } else if (inPoolBlock) {
      if (raw.trim().length > 0) {
        const entry = parsePoolEntry(raw, sourceLine, errors);
        if (entry) declaredPools.push(entry);
      }
      payload = { kind: "Blank" };
    } else {
      payload = classifyLine(raw, sourceLine, errors);
    }
  ...
  return {
    tokens,
    errors,
    declaredPools: declaredPools.length > 0 ? declaredPools : undefined,
  };
}
