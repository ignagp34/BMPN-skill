...
const END_CAPABLE: ReadonlySet<EventSubtype> = new Set<EventSubtype>([
  ...
  "escalate",
  "escalated",
  // Throw-style typed events are valid end events too: a `(message …)`,
  // `(send …)` or `(signal …)` that terminates a trace becomes a message/signal
  // end event — the throw counterpart of the message/signal START events that
  // START_CAPABLE already supports. emit.ts emits the matching
  // <*EventDefinition/> regardless of node kind.
  "message",
  "signal",
  "send",
]);
...
/**
 * Index laneName → declaredPoolName from a V5 `== pools ==` block. First
 * declaration wins for a lane listed under more than one pool (the duplicate is
 * reported separately as POOL_MAP_DUPLICATE_LANE during validation).
 */
function buildLaneToDeclaredPool(declaredPools: DeclaredPool[]): Map<string, string> {
  const m = new Map<string, string>();
  for (const dp of declaredPools) {
    for (const lane of dp.lanes) {
      if (!m.has(lane)) m.set(lane, dp.name);
    }
  }
  return m;
}
...
function build(
  ...
  // V5 export-safe P1: a sequence flow that crosses a *declared* pool boundary
  // is not legal BPMN (control flow may not cross pools — that is what message
  // flow is for). Prune such edges from `flows` so they are never serialized as
  // an illegal cross-pool sequence flow, but retain them in
  // crossPoolControlFlows so validation can raise POOL_BOUNDARY_CONTROL_FLOW.
  // Pruning happens BEFORE implicit start/end synthesis so the now-disconnected
  // pools are completed automatically (no emit-side special-casing).
  let crossPoolControlFlows: SequenceFlow[] | undefined;
  if (program.declaredPools && program.declaredPools.length > 0) {
    const laneToPool = buildLaneToDeclaredPool(program.declaredPools);
    const kept: SequenceFlow[] = [];
    const crossed: SequenceFlow[] = [];
    for (const f of flows) {
      const sourcePool = flowNodes.get(f.sourceId)?.pool;
      const targetPool = flowNodes.get(f.targetId)?.pool;
  ...
  // spell one out.
  flowCounter = synthesizeImplicitStartEvents(flowNodes, flows, pools, ids, flowCounter);

  // Symmetrically, synthesise an implicit end event for any tail node (no
  // outgoing flow) that isn't already an end event. Every trace should
  // terminate in a circle even when the DSL doesn't spell one out — exactly
  // as start events are handled above.
  flowCounter = synthesizeImplicitEndEvents(flowNodes, flows, pools, ids, flowCounter);
...
function synthesizeImplicitEndEvents(
  flowNodes: Map<string, FlowNode>,
  flows: SequenceFlow[],
  pools: ResolvedPool[],
  ids: IdAllocator,
  ...
  let nextFlow = flowCounterStart;
  for (const node of [...flowNodes.values()]) {
    if (node.kind === "endEvent") continue;
    // Boundary events ARE included here (unlike start-event synthesis): a boundary
    // event with no outgoing flow is an incomplete interrupt/escalation path.
    // Completing it with an implicit end event (a) makes the model semantically
    // whole and (b) avoids a hard crash in bpmn-auto-layout 0.5.0, whose
    // attachersHandler calls `.reverse()` on a boundary event's (undefined)
    // outgoing list. The synthesized end keeps the `EndEvent_implicit` prefix, so
    // validation surfaces it as the distinct IMPLICIT_BOUNDARY_END_EVENT diagnostic
    // and it stays out of the explicit-end count.
    if (node.kind === "dataObject" || node.kind === "dataStore") continue;
    if ((outgoingCount.get(node.id) ?? 0) > 0) continue;
    // Mirror of the start-event guard: a synthesized gateway that already has
    // incoming flows is a legitimate mid-flow router whose outgoing edges were
    // rewired (e.g. a split feeding `split → lane`), not a genuine trace tail.
    // Capping it with an end event would mask a real GATEWAY_WITHOUT_OUTGOING
    // problem, so leave it for validation to report.
    if (
      (node.kind === "exclusiveGateway" ||
        node.kind === "parallelGateway" ||
        node.kind === "eventBasedGateway") &&
    ...
    const endId = ids.for("EndEvent", `implicit::${node.id}`);
    flowNodes.set(endId, {
      id: endId,
      kind: "endEvent",
      pool: node.pool,
    ...
    const target = pools.find((p) => p.name === node.pool);
    if (target) target.nodeIds.push(endId);
    nextFlow++;
    flows.push({
      id: `Flow_${nextFlow}`,
      sourceId: node.id,
...
export function resolve(program: Program): ResolvedModel {
  ...
  // Resolve final pool assignments BEFORE attaching boundary events: the
  // boundary's edge-override key embeds its host task's mergeKey, so the host's
  // pool must be finalized first. Otherwise a host whose pool is fixed up by
  // first-mention (e.g. a fragment task that starts pool-less once a blank line
  // resets the default pool) gets a stale override key, leaving the
  // boundary→continuation flow dangling (unresolved source) and dropped on emit.
