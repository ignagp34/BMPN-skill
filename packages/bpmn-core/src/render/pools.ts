 ...
 * end up on the same y row. Lanes must form horizontal strips and each
 * task must sit in its own lane.
 *
 * This pass is the "snap to lane gridlines" step:
 *
...
export async function placePoolsAndLanes(layoutXml: string): Promise<string> {
  ...
  // Re-root the diagram plane onto the COLLABORATION. bpmn-auto-layout leaves the
  // plane pointing at the first <bpmn:process> (e.g. bpmnElement="Process_Employee").
  // With a process-rooted plane, bpmn-js — and strict importers like Bizagi —
  // render only that one process's flow nodes/lanes and silently drop every
  // participant pool box plus all other participants. Pointing the plane at the
  // collaboration makes the participant + lane shapes we just emitted authoritative,
  // so all pools render (stacked) in-app and on export.
  dg.plane.bpmnElement = collaboration;
  dg.id = `BPMNDiagram_${collaboration.id}`;
  dg.plane.id = `BPMNPlane_${collaboration.id}`;
...
export async function extendOuterLanes(layoutXml: string): Promise<string> {
  ...
  for (const el of planeElements) {
    ...
    // Only INTRA-participant edges (sequence flows routed within one pool) define
    // how far that pool must stretch vertically. CROSS-participant edges
    // (message flows) legitimately span pools — their far waypoints sit inside a
    // neighbouring pool, and using them here would drag this pool's top/bottom
    // into its neighbour, overlapping the two pools. Skip them.
    if (!srcPid || srcPid !== tgtPid) continue;
    const pid = srcPid;
    for (const wp of wps) {
